...
func (u *User) GetUserPosts(userID int64) ([]*app.UserPosts, error) {
  ...
  test := fmt.Sprintf("SELECT po.`post_title`, po.`post_body`, po.`created_at`, po.`updated_at`, u.`user_type`, u.`email`, u.`username` FROM posts as po, users as u WHERE po.`creator_id` = u.`id` AND u.`id` = %d;", userID)
